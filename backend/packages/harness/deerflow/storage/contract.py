...
class BlobRef(BaseModel):
    ...
    def _kind_is_safe(cls, value: str) -> str:
        if not is_valid_blob_kind(value):
            raise ValueError("kind must match ^[a-z0-9]([a-z0-9-]*[a-z0-9])?$ (it is used as a path segment)")
        return value
...
class BlobStore(ABC):
    ...
    def put_bytes(
        ...
        *,
        ...
        Idempotent: putting identical bytes twice returns the same ref and
        must not duplicate storage. ``thread_id`` is advisory metadata used by
        garbage collection (the retention contract in #5255 is per-thread);
        backends that do not need it may ignore it.
        """
