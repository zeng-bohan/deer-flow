...
(``content_type``, ``thread_id``, creation time). Reads do not depend on it: a
lost or truncated sidecar must not make blob content unreadable.
...
from deerflow.storage.contract import BlobNotFoundError, BlobReadError, BlobRef, BlobStore, BlobWriteError, is_valid_blob_kind

logger = logging.getLogger(__name__)

_META_SUFFIX = ".json"
_KIND_MAX_BYTES = 64 * 1024 * 1024  # defense-in-depth cap on a single put
    ...
    def put_bytes(
        ...
        *,
        ...
        if not is_valid_blob_kind(kind):
            raise ValueError("kind must match ^[a-z0-9][a-z0-9-]{0,63}$ (it is used as a path segment)")
        if len(data) > _KIND_MAX_BYTES:
            raise BlobWriteError(f"Refusing to store {len(data)} bytes (cap is {_KIND_MAX_BYTES})")
    ...
    def delete(self, ref: BlobRef) -> None:
        ...
        for path in (data_path, meta_path):
            try:
                ...
                raise BlobReadError(f"Failed to delete blob {ref.sha256[:12]}: {exc}") from exc
        if not removed:
